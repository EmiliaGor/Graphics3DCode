...
void SimpleShapeApplication::init() {
     ...
     * directive.
     */
    // This specifies that the data for attributes 0 and 1 should be read from a vertex buffer
    OGL_CALL(glEnableVertexAttribArray(0));
    OGL_CALL(glEnableVertexAttribArray(1));
    // and this specifies the data layout in the buffers.
    OGL_CALL(glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 6 * sizeof(GLfloat),
                                   reinterpret_cast<GLvoid *>(0)));
    OGL_CALL(glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, 6 * sizeof(float), (void*)(3* sizeof(float))));
    OGL_CALL(glEnableVertexAttribArray(1));
